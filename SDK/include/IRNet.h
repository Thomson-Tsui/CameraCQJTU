...
/**
* @brief 二次校准回调
* @param[in]    handle      IRNET_ClientSecondCalib4TmStart返回值
* @param[in]    m_ch        通道
* @param[in]    status_code 状态码(0-成功,否则失败)
* @param[in]    userdata    用户数据
*/
...
enum MessageOpt
{
    ...
    MESSAGE_CMD_GET_DAYNIGHT_STATE                  = 414,      /**<昼夜状态*/
    MESSAGE_CMD_GET_FLAME_DETECT_PARAM              = 415,      /**<火焰检测参数*/
    MESSAGE_CMD_SET_FLAME_DETECT_PARAM              = 416,      /**<火焰检测参数*/
    MESSAGE_CMD_FLAME_DETECT_PARAM_RESET            = 417,      /**<火焰检测参数复位*/
	MESSAGE_CMD_SET_GRIDSPARAM                      = 418,      /**<定制宫格功能*/
	MESSAGE_CMD_GET_GRIDSPARAM                      = 419,      /**<定制宫格功能*/
	MESSAGE_CMD_GET_GETREGIONTEMPVALUE_EX2          = 420,      /**<获取区域测温温度值(64个区域)*/
	MESSAGE_CMD_SET_MANUAL_FOCUS                    = 421,      /**<手动聚焦开始*/
	MESSAGE_CMD_SET_MANUAL_FOCUS_STOP               = 422,      /**<手动聚焦结束*/
	MESSAGE_CMD_GET_FOCUSPOS                        = 423,      /**<获取聚焦位置*/
	MESSAGE_CMD_SET_FOCUSPOS                        = 424,      /**<设置聚焦位置*/
	MESSAGE_CMD_SET_FOCUSCTRL                       = 425,      /**<自动对焦控制*/
	MESSAGE_CMD_GET_IMGFREEZE                       = 426,      /**<获取图像冻结控制*/
	MESSAGE_CMD_SET_IMGFREEZE                       = 427,      /**<设置图像冻结控制*/
	MESSAGE_CMD_GET_EXPOSUREPARAMEX                 = 428,      /**<获取曝光参数扩展*/
	MESSAGE_CMD_SET_EXPOSUREPARAMEX                 = 429,      /**<设置曝光参数扩展*/
	MESSAGE_CMD_GETVIPARAM                          = 430,      /**<获取视频参数*/
	MESSAGE_CMD_SETVIPARAM                          = 431,      /**<设置视频参数*/
	MESSAGE_CMD_GET_AGCPARAM_EX                     = 432,      /**<获取红外AGC参数*/
	MESSAGE_CMD_SET_AGCPARAM_EX                     = 433,      /**<设置红外AGC参数*/
	MESSAGE_CMD_GET_TEMPPARAM_EX                    = 434,      /**<获取测温相关参数*/
	MESSAGE_CMD_SET_TEMPPARAM_EX                    = 435,      /**<设置测温相关参数*/
	MESSAGE_CMD_GET_GETREGIONTEMPVALUE_EX           = 436,      /**<获取区域测温温度值(24个区域)*/

    MESSAGE_CMD_MAX                                 = 437,      /**<上限*/
};
 ...
 * @param[in] m_password 密码,小于等于20个字符
 * @param[in] pbuff      数据指针
 * @param[in] size       数据长度（宽*高*sizeof（char））
 ...
 * @param[in]     m_password 密码，小于等于20个字符
 * @param[in,out] pbuff      数据指针
 * @param[in]     size       pbuff大小（宽*高*sizeof（char））
 * @param[in]     wserport   设备端口
 * @return 大于等于0表示成功，返回数据长度，小于0表示失败
 * @attention 
*/
 ...
 * @param[in]    m_password 密码，小于等于20个字符
 * @param[in,out] pbuff     数据指针
 * @param[in]    size       pbuff大小（宽*高*sizeof（float））
 * @param[in]    wserport   设备端口
 * @return 大于等于0表示成功，返回数据长度，小于0表示失败
*/
...
* @param[in] m_password       连接密码，小于等于20字符
* @param[in] wserport         连接端口
* @param[in] seccalibcallback 校准回调函数
* @param[in] userdata         用户自定义参数
* @return -1 连接失败
...
/**
* @brief 二次标定执行(TM)
* @param[in] hHandle        连接句柄，IRNET_ClientSecondCalib4TmStart返回值
* @param[in] channel        通道号
* @param[in] calib_temp     标定温度(℃)
* @return 0表示成功，否则表示失败
